...
| pods `ImagePullBackOff` on 734 images | umbrella 0.1.18+ pulls images via the worker NODE role (kubelet ECR credential provider, no `aws-ecr-docker-creds`); the 734 ECR grant for the node role is missing, or the tenant is still on legacy 0.1.17 | onboard-customer-account grants the node role cross-account pull (§0.1) — ✅ node-role pull, no secret to mint, survives scale-to-zero. Legacy 0.1.17 tenants still use the old `aws-ecr-docker-creds` seeding |
