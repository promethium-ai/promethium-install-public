...
Run these checks from the install VM before starting the deployment. Every check should pass before proceeding.

### Outbound connectivity

```bash
curl -s https://github.com > /dev/null && echo "GitHub reachable" || echo "GitHub BLOCKED"
curl -s https://ghcr.io > /dev/null && echo "GHCR reachable" || echo "GHCR BLOCKED"
curl -s https://registry.terraform.io > /dev/null && echo "Terraform registry reachable" || echo "Terraform registry BLOCKED"
```
...
### Application Gateway subnet is empty

The App Gateway subnet must have no existing resources. This should return empty:
...
### Python venv and boto3

```bash
ls -la /tmp/venv/.venv/bin/activate
/tmp/venv/.venv/bin/python3 -c "import boto3; print(boto3.__version__)"
```
